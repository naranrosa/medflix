...
/* ============================================= */
/* --- Estilos da Semana Integradora --- */
/* ============================================= */

.integrator-week-view {
    padding-bottom: 4rem; /* Garante espaço no final da página */
}
...
.question-input-group {
    display: grid;
    grid-template-columns: 1fr auto auto; /* Colunas para texto, select e botão */
    gap: 1rem;
    align-items: center;
    margin-bottom: 1rem;
    padding: 1rem;
...
.question-input-group textarea {
    min-height: 48px; /* Altura mínima para o campo de texto */
    resize: vertical; /* Permite redimensionar verticalmente */
    width: 100%;
    padding: 0.75rem; /* Ajuste no padding interno */
    line-height: 1.5;
}

.question-input-group .select-input {
    min-width: 200px; /* Largura mínima para o seletor */
    height: 48px;
}
...
.answer-card h3 {
    ...
    margin-bottom: 1rem;
    font-size: 1.25rem;
    color: var(--primary-color); /* Destaque para a pergunta */
    font-weight: 600;
    line-height: 1.4;
}
...
.answer-reference p {
    ...
    font-size: 0.85rem;
    color: var(--text-secondary-color);
    white-space: pre-wrap; /* Quebra de linha para referências longas */
    word-wrap: break-word;
}
...
@media (max-width: 768px) {
    ...
    .question-input-group .icon-btn {
        position: absolute; /* Posiciona o botão de deletar */
        top: 0.75rem;
        right: 0.75rem;
    }
    .question-input-group {
        position: relative;
        padding-top: 3rem; /* Espaço para o botão de deletar não sobrepor o conteúdo */
    }
...
/* ==========================================================================
   CSS PARA AGENDAMENTO DE AULAS & CHAT
   ========================================================================== */

/* --- Botão Flutuante (FAB) para "Agendar Aula" --- */
.fab {
...
/* --- Tela de Lista de Agendamentos (Conversas) --- */
.schedules-view .dashboard-header {
  margin-bottom: 2rem;
}
...
.conversation-list {
  display: flex;
  flex-direction: column;
  gap: 1rem; /* Espaço entre os itens da lista */
}
...
.conversation-item-header {
  ...
  align-items: center;
  margin-bottom: 0.5rem;
  flex-wrap: wrap; /* Garante que funcione em telas menores */
  gap: 0.5rem;
}
...
.chat-view {
  display: flex;
  flex-direction: column;
  /* Ocupa a altura da tela, descontando o header principal se houver */
  height: calc(100vh - 80px);
  max-width: 900px;
  margin: 0 auto;
}

.chat-view .dashboard-header {
  display: flex;
  align-items: center;
  gap: 1rem;
  padding-bottom: 1rem;
...
.chat-view .dashboard-header h2 {
  margin: 0;
  font-size: 1.25rem;
  flex-grow: 1; /* Título ocupa o espaço disponível */
}

.chat-view .dashboard-header span {
  color: var(--text-secondary);
}

.chat-messages {
  flex-grow: 1; /* Faz a área de mensagens ocupar o espaço vertical */
  overflow-y: auto; /* Adiciona scroll se as mensagens ultrapassarem a tela */
  padding: 1.5rem 1rem;
  display: flex;
  flex-direction: column;
  gap: 1rem;
...
.chat-message {
  display: flex;
  flex-direction: column;
  max-width: 75%; /* Mensagens não ocupam a tela inteira */
...
/* Mensagem Recebida (da outra pessoa) */
...
.chat-message.received p {
  ...
  border: 1px solid var(--border-color);
  color: var(--text-primary);
  border-top-left-radius: 0; /* Efeito de "balão" de chat */
}
...
.chat-message.sent p {
  background-color: var(--primary-color);
  color: white;
  border-top-right-radius: 0; /* Efeito de "balão" de chat */
}
...
/* ==========================================================================
   CSS PARA FUNCIONALIDADES AVANÇADAS DE ESTUDO (CATEGORIA 1)
   ========================================================================== */

/* --- 1. MELHORIAS NOS FLASHCARDS (SISTEMA SRS) --- */
...
.flashcard-mode-selection .btn {
  padding: 1rem;
  font-size: 1.1rem;
}


/* --- 2. ANOTAÇÕES PESSOAIS NOS RESUMOS --- */

/* Botão pop-up que aparece ao selecionar texto */
.annotation-creator-btn {
  position: absolute; /* Posição calculada via JS */
  z-index: 1050;
  padding: 0.5rem 1rem;
  font-size: 0.875rem;
  background-color: var(--primary-color);
...
.annotation-creator-btn:hover {
  transform: scale(1.05);
  background-color: var(--primary-hover);
}

/* Painel lateral que exibe as anotações */
.annotations-panel {
  width: 300px;
  flex-shrink: 0;
  padding: 1.5rem 1rem;
...
/* Estilo para um item de anotação na lista */
.annotation-item {
  background-color: var(--card-bg);
  border: 1px solid var(--border-color);
  border-radius: var(--border-radius-lg);
...
/* Estilo para a citação dentro do modal de criação de anotação */
.highlighted-quote-modal {
  margin-bottom: 1rem;
  padding: 1rem;
  font-style: italic;
  color: var(--text-secondary);
...
/* Ícone de anotações no cabeçalho do resumo */
.summary-view-toggles {
  display: flex;
  gap: 0.5rem;
}
.summary-view-toggles .icon-btn.active {
  background-color: var(--primary-color);
  color: white;
}
...
.subjects-checkbox-group label:hover {
    background-color: var(--card-bg-hover);
}

.subjects-checkbox-group input[type="checkbox"] {
    width: 18px;
    height: 18px;
...
.mock-exam-view, .mock-exam-results {
  padding: 2rem;
  max-width: 800px;
  margin: auto;
}
...
.mock-exam-header span {
  font-size: 1rem;
  color: var(--text-secondary);
}

.mock-exam-view .alternatives-list .alternative-item.selected {
    background-color: var(--primary-color);
    color: white;
    border-color: var(--primary-color);
}
...
/* ==========================================================================
   ESTILOS ESSENCIAIS PARA O GERENCIAMENTO DE DPs
   ========================================================================== */

/* --- Modal de Gerenciamento de DPs --- */
/* Garante que o container de matérias seja rolável e tenha um bom visual */
.dp-selection-container {
  max-height: 60vh; /* Altura máxima antes de a barra de rolagem aparecer */
  overflow-y: auto;
  padding: 1rem;
  border: 1px solid var(--border-color);
  border-radius: 8px;
...
/* --- Agrupamento de Matérias por Período --- */
.dp-term-group {
  border: none;
  /* Cria uma linha lateral para separar visualmente os períodos */
  border-left: 2px solid var(--border-color);
  padding: 1rem 1.5rem;
  margin-bottom: 1.5rem;
}
.dp-term-group:last-child {
  margin-bottom: 0;
}

.dp-term-group legend {
  padding: 0 0.5rem;
  margin-left: -0.5rem; /* Alinha com a borda esquerda */
  font-weight: 600;
  font-size: 1.1rem;
  color: var(--primary-accent); /* Usa a cor de destaque do seu tema */
}

/* --- Grade de Matérias (Checkboxes) --- */
.subjects-checkbox-group {
  display: grid;
  /* Cria colunas flexíveis, ideal para responsividade em diferentes tamanhos de modal */
  grid-template-columns: repeat(auto-fill, minmax(250px, 1fr));
  gap: 0.75rem;
  margin-top: 1rem;
}

/* --- Estilo de Cada Matéria na Lista --- */
.dp-subject-label {
  display: flex;
  align-items: center;
  padding: 0.75rem;
...
.dp-subject-label:hover {
  background-color: var(--icon-button-bg-hover);
  border-color: var(--border-color);
}

/* Estilo para matérias desabilitadas (do período principal do aluno) */
.dp-subject-label.disabled {
  cursor: not-allowed;
  color: var(--text-color);
  opacity: 0.6;
  background-color: transparent;
...
.dp-subject-label input[type="checkbox"] {
  margin-right: 0.75rem;
  width: 18px;
  height: 18px;
  accent-color: var(--primary-accent); /* Pinta o checkbox com a cor primária */
}

/* Estilo para o pequeno texto "(Período Principal)" */
.main-term-indicator {
  font-size: 0.75rem;
  color: var(--text-color);
  opacity: 0.7;
...
/* --- Estilo para garantir que o botão "Gerenciar DPs" se pareça com os outros --- */
.admin-table .user-actions .btn-secondary {
    background-color: var(--card-color);
    color: var(--primary-text-color);
    border: 1px solid var(--border-color);
}
.admin-table .user-actions .btn-secondary:hover {
    background-color: var(--background-color);
}

/* ==========================================================================
   ESTILOS PARA ANOTAÇÃO COM CANETA (CANVAS)
   ========================================================================== */

/* Container que segura tanto o conteúdo do resumo quanto o canvas */
.annotation-container {
  position: relative; /* Essencial para o posicionamento absoluto do canvas */
  width: 100%;
}

/* O canvas que ficará por cima */
.annotation-canvas {
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
  height: 100%;
  cursor: crosshair; /* Muda o cursor para indicar que é uma área de desenho */
}
...
.annotation-toolbar .icon-btn.active {
  background-color: var(--primary-accent);
  color: white;
}

/* Estilo para as amostras de cor */
.color-swatch-btn {
  width: 32px;
  height: 32px;
  border-radius: 50%;
